"""
Plan Validator - Validates Structured Execution Plans
Ensures plans meet strict schema requirements before execution.
"""
...
class PlanValidator:
    """Validates execution plans against strict schema"""
    ...
    def validate_plan(self, plan: Dict[str, Any]) -> ValidationResult:
        ...
        elif plan['operation'] not in self.VALID_OPERATIONS:
            errors.append(
        ...
        if 'filters' in plan:
            if not isinstance(plan['filters'], list):
                ...
                for i, filter_obj in enumerate(plan['filters']):
                    filter_errors = self._validate_filter(filter_obj, i)
        ...
        if 'chart_type' in plan:
            if plan['chart_type'] not in self.VALID_CHART_TYPES:
                errors.append(
                    f"Invalid chart_type '{plan['chart_type']}'. "
                    f"Must be one of: {', '.join(str(ct) for ct in self.VALID_CHART_TYPES if ct)}"
                )
        ...
        if operation == 'forecast':
            if not plan.get('time_column'):
                errors.append(
                    "Operation 'forecast' requires time_column"
                )
            if not plan.get('target_column'):
                errors.append(
                    "Operation 'forecast' requires target_column"
                )
    ...
    def _validate_filter(self, filter_obj: Any, index: int) -> List[str]:
        ...
        elif filter_obj['operator'] not in self.VALID_OPERATORS:
            errors.append(
