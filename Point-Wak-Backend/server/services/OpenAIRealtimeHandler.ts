...
/**
 * Handler functions for AI function calls
 * Each handler is responsible for a single operation
 */
const functionHandlers = {
  async lookupProperty(args: any, session: RealtimeSession) {
    return await rulesEngine.resolvePropertyAndUnit(args.query, session.accountId);
  },

  async lookupUnit(args: any, session: RealtimeSession) {
    return await storage.getUnitByNumber(args.propertyId, args.unitNumber);
  },

  async classifyIssue(args: any, session: RealtimeSession) {
    return rulesEngine.classifyTradeAndSeverity(args.description);
  },

  async createIncident(args: any, session: RealtimeSession) {
    ...
    return { incidentId: incident.id, message: "Incident created successfully" };
  },

  async dispatchVendor(args: any, session: RealtimeSession) {
    const incident = await storage.getIncident(args.incidentId);
    if (!incident) {
      return { error: "Incident not found" };
    }

    const vendorSelection = await rulesEngine.selectVendor(
      incident.trade || "general",
      incident.propertyId || "",
      session.accountId,
      false
    );
    ...
    if (!vendorSelection.vendor) {
      return { error: vendorSelection.error || "No vendor available for this trade" };
    }
  ...
  async transferToHuman(args: any, session: RealtimeSession) {
    const users = await storage.getUsers(session.accountId);

    // Try to find on-call user first
    ...
    if (onCallUser?.phoneNumber) {
      ...
      return { transferred: true, to: "on-call agent" };
    }

    // Fall back to any available agent
    const availableAgent = users.find(u => u.availability === "available" && u.phoneNumber);
    if (availableAgent?.phoneNumber) {
      await telephonyAdapter.transferCall(session.callSession.id, availableAgent.phoneNumber);
      return { transferred: true, to: "available agent" };
    }

    return { transferred: false, message: "No agents available, taking a message" };
  },
};

type FunctionHandler = (args: any, session: RealtimeSession) => Promise<any>;
...
async function executeFunctionCall(
  ...
  const handler = functionHandlers[functionName as keyof typeof functionHandlers] as FunctionHandler | undefined;
 ...
 */
function sendFunctionResult(session: RealtimeSession, callId: string, result: any): void {
  if (!session.openaiWs || session.openaiWs.readyState !== WebSocket.OPEN) {
    return;
  }
...
/**
 * Main handler for function calls from OpenAI
 */
